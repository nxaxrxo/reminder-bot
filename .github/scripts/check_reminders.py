...
def calc_next(t, repeat):
    ...
    m = re.match(r'^(\d+)min$', repeat)
    if m:
        return t + timedelta(minutes=int(m.group(1)))
    m = re.match(r'^(\d+)h$', repeat)
    if m:
    ...
    if remind_time <= now:
        ...
        if sent:
            ...
            if repeat:
                ...
                remaining.append(r)
                pending_count += 1
                print("  Next: " + r["id"] + " -> " + next_t.isoformat() + " (" + repeat + ")")
        else:
            print("Telegram fail: " + r["id"] + ", will retry")
            remaining.append(r)
            pending_count += 1
    else:
        remaining.append(r)
...
if sent_count > 0 or len(remaining) != len(data.get("reminders", [])):
    ...
    print("Updated reminders.json")

print("Done. " + str(sent_count) + " sent, " + str(pending_count) + " pending.")